...
% - Carozza, D. A.; Bianchi, D. & Galbraith, E. D. 
% Formulation, General
% Features and Global Calibration of a 
% Bioenergetically-Constrained Fishery Model.
% PLOS ONE, Public Library of Science, 2017, 12, 1-28

% This model version is edited by Kim Scherrer, to add regulation to the
% economic module
%**************************************************************************
% Inputs:
%   - parameters.txt
...
%**************************************************************************
 if boats.param.main.param_ens 
    ...
    % MODIFY ENSEMBLE PARAMETERS TO HAVE IRON LIMIT. (determined by kfe) --
    ens_param = modify_Fe_param(boats.param.ecology.kfe, boats.param.path.wrkdir); 
    % ---------------------------------------------------------------------
...
for indr = 1:param_loops
   ...
   if boats.param.main.param_ens
     ...
     boats.param = modify_parameters(boats,...
                    ...
                    'te',squeeze(ens_param.te_no3(indr,:,:)),...            % MODIFIED TO HAVE IRON LIMITATION!! (original 'ens_param.te(indr)')
                    'ppmr',ens_param.ppmr(indr), ...
                    'tro_sca',squeeze(ens_param.tro_sca_no3(indr,:,:)), ... % MODIFIED TO HAVE IRON LIMITATION!! (original 'ens_param.tro_sca(indr)')
   ...
   boats.forcing = load_forcing(boats,...
                     forcing_ecological,forcing_economical,indr);
